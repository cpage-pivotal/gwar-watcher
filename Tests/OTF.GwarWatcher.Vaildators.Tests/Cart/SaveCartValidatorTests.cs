...
{
    [TestClass]
    public class SaveCartValidatorTests : ValidatorTestsBase
    {
        [TestMethod]
        public void Validate_Valid()
        {
            ValidatorProvider p = new ValidatorProvider();
            this.GetMessages(Data.Cart.SaveCart_Valid).ForEach(m =>
            {
                ValidatorResult result = p.Validate(m);
                m.EventId.ShouldEqual(21);
        ...
        {
            ValidatorProvider p = new ValidatorProvider();
            this.GetMessages(Data.Cart.SaveCart_Invalid).ForEach(m =>
            {
                ValidatorResult result = p.Validate(m);
                m.EventId.ShouldEqual(21);
                result.ShouldNotBeNull();
