...
{
    [TestClass]
    public class ClearCartValidatorTests : ValidatorTestsBase
    {
        [TestMethod]
        public void Validate_Valid()
        {
            ValidatorProvider p = new ValidatorProvider();
            this.GetMessages(Data.Cart.ClearCart_Valid).ForEach(m =>
            {
                ValidatorResult result = p.Validate(m);
                m.EventId.ShouldEqual(20);
        ...
        {
            ValidatorProvider p = new ValidatorProvider();
            this.GetMessages(Data.Cart.ClearCart_Invalid).ForEach(m =>
            {
                ValidatorResult result = p.Validate(m);
                m.EventId.ShouldEqual(20);
                result.ShouldNotBeNull();
