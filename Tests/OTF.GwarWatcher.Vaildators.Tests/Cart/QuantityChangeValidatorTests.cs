...
{
    [TestClass]
    public class QuantityChangeValidatorTests : ValidatorTestsBase
    {
        [TestMethod]
        public void Validate_Valid()
        {
            ValidatorProvider p = new ValidatorProvider();
            this.GetMessages(Data.Cart.QuantityChange_Valid).ForEach(m =>
            {
                ValidatorResult result = p.Validate(m);
                m.EventId.ShouldEqual(23);
        ...
        {
            ValidatorProvider p = new ValidatorProvider();
            this.GetMessages(Data.Cart.QuantityChange_Invalid).ForEach(m =>
            {
                ValidatorResult result = p.Validate(m);
                m.EventId.ShouldEqual(23);
                result.ShouldNotBeNull();
