...
{
    [TestClass]
    public class AdImpressionValidatorTests : ValidatorTestsBase
    {
        [TestMethod]
        public void Validate_Valid()
        {
            ValidatorProvider p = new ValidatorProvider();
            this.GetMessages(Data.Pdp.AdImpression_Valid).ForEach(m =>
            {
                ValidatorResult result = p.Validate(m);
                m.EventId.ShouldEqual(19);
        ...
        {
            ValidatorProvider p = new ValidatorProvider();
            this.GetMessages(Data.Pdp.AdImpression_Invalid).ForEach(m =>
            {
                ValidatorResult result = p.Validate(m);
                m.EventId.ShouldEqual(19);
                result.ShouldNotBeNull();
