...
{
    [TestClass]
    public class EmailValidatorTests : ValidatorTestsBase
    {
        [TestMethod]
        public void Validate_Valid()
        {
            ValidatorProvider p = new ValidatorProvider();
            this.GetMessages(Data.Pdp.Email_Valid).ForEach(m =>
            {
                ValidatorResult result = p.Validate(m);
                m.EventId.ShouldEqual(9);
        ...
        {
            ValidatorProvider p = new ValidatorProvider();
            this.GetMessages(Data.Pdp.Email_Invalid).ForEach(m =>
            {
                ValidatorResult result = p.Validate(m);
                m.EventId.ShouldEqual(9);
                result.ShouldNotBeNull();
