...
{
    [TestClass]
    public class EmailProductValidatorTests : ValidatorTestsBase
    {
        [TestMethod]
        public void Validate_Valid()
        {
            ValidatorProvider p = new ValidatorProvider();
            this.GetMessages(Data.Pdp.EmailProduct_Valid).ForEach(m =>
            {
                ValidatorResult result = p.Validate(m);
                m.EventId.ShouldEqual(7);
        ...
        {
            ValidatorProvider p = new ValidatorProvider();
            this.GetMessages(Data.Pdp.EmailProduct_Invalid).ForEach(m =>
            {
                ValidatorResult result = p.Validate(m);
                m.EventId.ShouldEqual(7);
                result.ShouldNotBeNull();
