...
{
    [TestClass]
    public class ChatValidatorTests : ValidatorTestsBase
    {
        [TestMethod]
        public void Validate_Valid()
        {
            ValidatorProvider p = new ValidatorProvider();
            this.GetMessages(Data.Pdp.Chat_Valid).ForEach(m =>
            {
                ValidatorResult result = p.Validate(m);
                m.EventId.ShouldEqual(8);
        ...
        {
            ValidatorProvider p = new ValidatorProvider();
            this.GetMessages(Data.Pdp.Chat_Invalid).ForEach(m =>
            {
                ValidatorResult result = p.Validate(m);
                m.EventId.ShouldEqual(8);
                result.ShouldNotBeNull();
